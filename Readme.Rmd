...
KNN(dat = word_distance_lakok, variation_type = 'lakok/lakom', distance_type = 'phon', var_s = .1, var_k = 3, var_p = 1)
...
The GCM models are all pretty good. For 'cselekszenek' and 'lakok', the best model uses phonological distance. For 'hotelban' it, again, uses jaccard distance.
...
```{r best_models}
my_knns3 = my_knns2 %>% 
  select(sim,variation,estimate,std.error,statistic,sig) %>% 
  mutate(model = 'KNN')
