...
alignments = runLookup(test,training,'whatever',lookup)
...
knn_out1 = KNN(dat = word_distance, variation_type = 'whatever', distance_type = 'phon', var_s = .1, var_k = 3, var_p = 1)

# you can use a wrapper function to do a-e:
knn_out2 = KNNwrapper(test = test, training = training, feature_matrix = fm, my_variation = 'whatever', my_distance = 'phon', my_s = .1, my_k = 3, my_p = 1)

flag4 = all(knn_out1 == knn_out2)

# you can use the wrapper function to use some other distance and skip the whole alignment bit:
knn_out3 = KNNwrapper(test = test, training = training, feature_matrix = fm, my_variation = 'whatever', my_distance = 'jaccard', my_s = .1, my_k = 3, my_p = 1)

flag5 = any(knn_out2 != knn_out3)

flags = all(flag1,flag2,flag3,flag4,flag5)
